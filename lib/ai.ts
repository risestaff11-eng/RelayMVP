...
export async function generateStructuredJsonFromAudio<T>({
  ...
  if (!text) throw new Error(candidate?.finishMessage || "Rela не смогла расшифровать запись");
  let data: T;
  try { data = JSON.parse(text) as T; }
  catch { throw new Error("Rela подготовила расшифровку в неверном формате. Повторите запись"); }
