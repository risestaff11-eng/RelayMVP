...
type Row = { id: string; name: string; email: string; phone: string; company: string; createdAt: string; status: string; emailVerifiedAt: string | null; tokenBalance: number | null };
...
export function SystemUsers({ authorized, initialRows }: { authorized: boolean; initialRows: Row[] }) {
  ...
  return <main className="system-users"><h1>Пользователи</h1>{error && <p className="system-error">{error}</p>}<div className="system-table"><div className="system-row system-head"><span>Имя</span><span>Email</span><span>Подтверждение</span><span>Телефон</span><span>Компания</span><span>Дата регистрации</span><span>Статус</span><span>AI-кредиты</span><span>Действия</span></div>{rows.map((row) => <div className="system-row" key={row.id}><span data-label="Имя">{row.name}</span><span data-label="Email">{row.email}</span><span data-label="Подтверждение"><b className={`system-email-status ${row.emailVerifiedAt ? "verified" : "unverified"}`}>{row.emailVerifiedAt ? "Email подтверждён" : "Не подтверждён"}</b></span><span data-label="Телефон">{row.phone || "—"}</span><span data-label="Компания">{row.company || "—"}</span><span data-label="Дата регистрации">{formatDateTime(row.createdAt)}</span><span data-label="Статус"><b className={`system-status status-${row.status}`}>{row.status}</b></span><span className="system-token-cell" data-label="AI-кредиты"><b>{formatInteger(row.tokenBalance ?? 0)}</b><label><input type="number" min="1" max="10000000" placeholder="Добавить" value={tokenAmounts[row.id] ?? ""} onChange={(event) => setTokenAmounts((current) => ({ ...current, [row.id]: event.target.value }))} /><button type="button" disabled={busy === row.id} onClick={() => void addTokens(row.id)}>+</button></label></span><span className="system-actions" data-label="Действия"><button className={row.status === "active" ? "is-active" : "needs-activation"} type="button" disabled={busy === row.id || row.status === "active"} onClick={() => update(row.id, "active")}>{row.status === "active" ? "Активирован" : "Активировать вручную"}</button><button className={row.status === "blocked" ? "is-blocked" : ""} type="button" disabled={busy === row.id || row.status === "blocked"} onClick={() => update(row.id, "blocked")}>{row.status === "blocked" ? "Заблокирован" : "Заблокировать"}</button><button type="button" disabled={busy === row.id} onClick={() => remove(row.id)}>Удалить</button></span></div>)}</div>{rows.length === 0 && <p>Заявок пока нет.</p>}</main>;
}
