...
const faqs = [
  ...
  ["Что получает компания после регистрации?", "После активации в кабинете доступны программы, кабинеты агентов и стартовый резерв 50 000 AI-кредитов для анализа сайта, подготовки профиля и создания первых программ."],
];
  ...
  return <main className="lp-shell">
    ...
    <section className="lp-hero" id="main-content">
      ...
      <div className="lp-hero-copy">
        ...
        <div className="lp-hero-actions"><a className="lp-primary" href={dashboardHref} data-track="hero_primary">{user ? "Открыть кабинет" : "Запустить первую программу"}<span>↗</span></a><a className="lp-secondary" href="#product" data-track="hero_secondary">Посмотреть продукт <span>↓</span></a></div>
        <p className="lp-cta-note">50 000 AI-кредитов на старте · агентам не нужно приложение · заявку проверяем перед активацией</p>
    ...
    <section className="lp-offer lp-section" id="offer"><div className="lp-offer-copy"><span>50 000 AI-КРЕДИТОВ НА СТАРТЕ</span><h2>Начните с готовым ресурсом для запуска агентского канала.</h2><p>После регистрации мы проверим заявку, активируем кабинет и начислим стартовый резерв для AI-настройки профиля и первых программ.</p><ul><li>50 000 AI-кредитов сразу после создания компании</li><li>Программы и задания для разных сценариев продаж</li><li>Кабинеты компании и агентов</li><li>Фиксация результатов, статусов и выплат</li></ul><div className="lp-offer-actions"><a className="lp-primary" href={dashboardHref} data-track="offer_primary">{user ? "Перейти к программам" : "Получить стартовый резерв"}<span>↗</span></a><Link href="/pricing" data-track="pricing_link">Посмотреть варианты подключения →</Link></div></div><ol className="lp-offer-steps"><li><b>01</b><span><strong>Зарегистрируйтесь</strong>Оставьте данные компании.</span></li><li><b>02</b><span><strong>Получите доступ</strong>Мы проверим заявку и откроем кабинет.</span></li><li><b>03</b><span><strong>Создайте программу</strong>Используйте AI-кредиты для настройки.</span></li><li><b>04</b><span><strong>Отправьте ссылку</strong>Пригласите первых агентов.</span></li></ol></section>
    ...
    <section className="lp-final"><div className="lp-final-tiles" aria-hidden="true">{missionCards.map((card) => <i className={`lp-${card.tone}`} key={card.index}>{card.icon}</i>)}</div><span>ПЕРВАЯ ПРОГРАММА НАЧИНАЕТСЯ С ОДНОЙ ССЫЛКИ</span><h2>Дайте агентам понятный повод рекомендовать вас.</h2><p>Получите 50 000 AI-кредитов на старте, настройте первую программу и пригласите агентов по одной ссылке.</p><a className="lp-primary" href={dashboardHref} data-track="final_primary">{user ? "Открыть кабинет" : "Запустить агентский канал"}<span>↗</span></a></section>
    <a className="lp-mobile-sticky-cta" href={dashboardHref} data-track="mobile_sticky">{user ? "Открыть кабинет" : "Запустить программу"}<span>↗</span></a>
