...
export default async function PublicProgramPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ access?: string }> }) {
  ...
  if (!authorized) return <PartnerEntry companyId={company.id} companyName={company.name} logoObjectKey={company.logoObjectKey} programName={program.name} reward={program.missions[0]?.rewardLabel || "По условиям задания"} />;
