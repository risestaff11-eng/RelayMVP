...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title: program?.name ?? "Партнёрская программа", description: program?.description ?? "Партнёрская программа в Relay", referrer: "no-referrer" };
}
...
export default async function PublicProgramPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ access?: string }> }) {
  ...
  const authorized = portal?.program.id === program.id ? portal : null;
  if (!authorized) return <PartnerEntry programSlug={slug} companyName={company.name} programName={program.name} reward={program.missions[0]?.rewardLabel || "По условиям миссии"} />;

  return <main className="partner-program-page"><header className="partner-program-nav"><Link className="brand" href={`/p/${slug}?access=${access}`}><span className="brand-mark">R</span><span>Relay</span></Link><div className="partner-program-nav-actions"><span>Партнёрская программа · {company.name}</span><Link href={`/partner/${access}`}>Кабинет партнёра →</Link></div></header><section className="partner-program-hero"><span className="live-pill">● ПРОГРАММА АКТИВНА</span><h1>{program.name}</h1><p>{program.description}</p><div className="partner-program-facts"><div><small>КОМПАНИЯ</small><strong>{company.name}</strong></div><div><small>МИССИЙ</small><strong>{program.missions.length}</strong></div><div><small>ВАЛЮТА</small><strong>{program.currency}</strong></div><div><small>СРОК</small><strong>{program.expiresAt ? new Date(program.expiresAt).toLocaleDateString("ru-RU") : "Без ограничения"}</strong></div></div><a className="button button-primary" href="#missions">Посмотреть миссии <span>↓</span></a></section><section className="partner-missions-section" id="missions"><div className="partner-section-heading"><span className="module-kicker">ШАГ 2 ИЗ 2</span><h2>Выберите и возьмите миссию</h2><p>После выбора откроется передача результата. Условия зафиксированы до начала работы.</p></div><div className="partner-mission-grid">{program.missions.map((mission, index) => <article className={`partner-mission-card type-${mission.type.toLowerCase()}`} key={mission.id}><div className="partner-mission-number">0{index + 1} · {typeNames[mission.type]}</div><h3>{mission.title}</h3><p>{mission.description}</p><div className="partner-reward"><small>ВОЗНАГРАЖДЕНИЕ</small><strong>{mission.rewardLabel}</strong></div><div className="partner-mission-block"><strong>Что сделать</strong><ol>{mission.instructions.map((item) => <li key={item}>{item}</li>)}</ol></div><div className="partner-mission-block"><strong>Что приложить</strong><ul>{mission.proofRequirements.map((item) => <li key={item}>{item}</li>)}</ul></div><div className="partner-verification"><strong>Как проверяется</strong><p>{mission.verificationRules}</p></div><PublicMissionAction token={access} missionId={mission.id} programSlug={slug} accepted={authorized.acceptances.some((item) => item.missionId === mission.id && item.status === "ACTIVE")} /></article>)}</div></section><section className="partner-terms"><div><span className="module-kicker">ПРОЗРАЧНЫЕ УСЛОВИЯ</span><h2>До начала миссии</h2></div><div><article><strong>Сроки выплаты</strong><p>{program.payoutTerms}</p></article><article><strong>Ограничения</strong><p>{program.legalTerms}</p></article></div><p className="partner-beta-note">Relay фиксирует владельца рекомендации, дату отправки и всю историю статусов.</p></section><footer className="partner-program-footer"><span>Работает на Relay</span><a href={company.website} target="_blank" rel="noreferrer" referrerPolicy="no-referrer">Сайт компании ↗</a></footer></main>;
}
