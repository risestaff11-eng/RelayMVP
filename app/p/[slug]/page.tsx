...
export default async function PublicProgramPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ access?: string }> }) {
  ...
  if (!authorized) return <PartnerEntry programSlug={slug} companyName={company.name} programName={program.name} reward={program.missions[0]?.rewardLabel || "По условиям задания"} />;
