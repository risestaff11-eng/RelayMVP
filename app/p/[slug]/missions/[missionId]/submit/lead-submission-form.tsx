...
export function LeadSubmissionForm({ programSlug, missionId, token }: { programSlug: string; missionId: string; token: string }) {
  ...
  async function nextStep() {
    ...
    if (String(values.get("contactPhone") || "").replace(/\D/g, "").length < 7) return setError("Укажите корректный телефон потенциального клиента");
