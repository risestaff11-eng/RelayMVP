...
const recoveryWhatsAppUrl = (email: string) => `https://wa.me/77765086000?text=${encodeURIComponent(`Не могу восстановить пароль Yaler. Email аккаунта: ${email}`)}`;
...
export function AuthFlow({ returnTo }: { returnTo: string }) {
  ...
  return <main className="relay-auth-page"><section className="relay-auth-card">
    <div className="relay-auth-brand"><MarketingLogo /><strong>Yaler</strong></div>
    {step === "email" && <><div className="relay-auth-copy"><h1>Вход в Yaler</h1><p>Введите email, чтобы войти или создать новый аккаунт.</p></div><form onSubmit={checkEmail}><label><span>Email</span><input autoComplete="email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@company.com" required /></label>{error && <p className="relay-auth-error" role="alert">{error}</p>}<button disabled={pending} type="submit">{pending ? "Проверяем…" : "Продолжить"}</button></form></>}
    ...
    {step === "reset-success" && <div className="relay-auth-success"><span>✓</span><h1>Пароль изменён</h1><p>Теперь войдите с новым паролем. Все старые сеансы закрыты для безопасности.</p><button className="relay-auth-primary" type="button" onClick={() => { setStep("login"); setError(""); }}>Войти в Yaler</button></div>}
