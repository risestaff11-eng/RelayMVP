...
export function AuthFlow({ returnTo }: { returnTo: string }) {
  ...
  function chooseEmail(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setDisplayName(email.split("@")[0]);
    setError("");
    const submitter = (event.nativeEvent as SubmitEvent).submitter as HTMLButtonElement | null;
    setStep(submitter?.name === "intent" && submitter.value === "register" ? "register" : "login");
  }
  ...
  return <main className="relay-auth-page"><section className="relay-auth-card">
    ...
    {step === "email" && <><div className="relay-auth-copy"><h1>Вход в RiseStaff</h1><p>Введите email, затем выберите вход или создание кабинета.</p></div><form onSubmit={chooseEmail}><label><span>Email</span><input autoComplete="email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@company.com" required /></label>{error && <p className="relay-auth-error" role="alert">{error}</p>}<button disabled={pending} type="submit" name="intent" value="login">Войти</button><button className="relay-auth-back" type="submit" name="intent" value="register">Создать аккаунт</button></form></>}
    {step === "login" && <><div className="relay-auth-copy"><h1>Вход в кабинет</h1><p>Введите пароль для {email}.</p></div><form onSubmit={login}><label><span>Пароль</span><input autoComplete="current-password" name="password" type="password" required /></label><button className="relay-auth-forgot" type="button" onClick={() => { setStep("forgot"); setError(""); }}>Забыли пароль?</button>{error && <p className="relay-auth-error" role="alert">{error}</p>}<button disabled={pending} type="submit">{pending ? "Входим…" : "Войти"}</button></form><button className="relay-auth-back" type="button" onClick={reset}>← Другой email</button></>}
