...
export default async function MaterialsPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <div className="partner-portal-content knowledge-base-page">
    <div className="partner-page-heading"><div><span>ПОЛЕВАЯ БАЗА</span><h1>Подготовьтесь к разговору о {<bdi data-no-translate>{portal.company.name}</bdi>}</h1><p>Идите по порядку: разберитесь в продукте, найдите подходящего клиента, проведите разговор и передайте результат.</p></div></div>

    <section className="agent-company-overview"><div><small>ВЫ ПРЕДСТАВЛЯЕТЕ</small><h2>{<bdi data-no-translate>{portal.company.name}</bdi>}</h2><p>{(portal.companyProfile?.businessDescription || portal.program.description) ? ((portal.companyProfile?.businessDescription) ? (portal.companyProfile?.businessDescription) : (<bdi data-no-translate>{portal.program.description}</bdi>)) : ("Описание компании пока не добавлено.")}</p></div><nav><a href={portal.company.website} target="_blank" rel="noreferrer">Сайт компании <span>↗</span></a>{instagram ? <a href={`https://instagram.com/${instagram}`} target="_blank" rel="noreferrer">Instagram @{instagram} <span>↗</span></a> : <span>Instagram не указан компанией</span>}{whatsappDigits ? <a href={`https://wa.me/${whatsappDigits}?text=${encodeURIComponent(`Здравствуйте! Я агент компании ${portal.company.name} в RiseStaff.`)}`} target="_blank" rel="noreferrer">WhatsApp компании <span>↗</span></a> : <span>WhatsApp не указан компанией</span>}</nav></section>
    ...
    {portal.knowledgeItems.length ? <>
      ...
      {grouped.map((group) => <section className="agent-knowledge-stage" id={`knowledge-${group.stage.toLowerCase()}`} key={group.stage}><header><span>{stageNames[group.stage]}</span><p>{group.stage === "PREPARE" ? "Сначала поймите продукт и границы обещаний." : group.stage === "OUTREACH" ? "Начните разговор без давления и получите разрешение продолжить." : group.stage === "QUALIFY" ? "Проверьте, подходит ли клиент, прежде чем передавать контакт." : group.stage === "PRESENT" ? "Свяжите потребность клиента с подтверждённой ценностью." : group.stage === "FOLLOW_UP" ? "Вернитесь к разговору с понятной причиной и следующим шагом." : "Зафиксируйте договорённость и корректно передайте результат."}</p></header><div className="company-knowledge-grid">{group.items.map((item) => <article key={item.id}>
          ...
          <div>{item.content && <CopyTextButton text={item.content} label="Скопировать" />}{item.externalUrl && <a href={item.externalUrl} target="_blank" rel="noreferrer">Открыть ссылку ↗</a>}{item.objectKey && <a href={`/api/partner/knowledge/${item.id}?token=${token}`}>Скачать {(item.fileName) ? (<bdi data-no-translate>{item.fileName}</bdi>) : ("файл")} ↓</a>}</div>
        </article>)}</div></section>)}
    ...
    <section className="materials-grid"><header className="agent-task-playbooks"><span>ПОД ВАШИ ЗАДАНИЯ</span><h2>Быстрый старт разговора</h2><p>Готовые сообщения и критерии по каждому доступному заданию.</p></header>{portal.missions.map((mission) => { const intro = `Здравствуйте! Хочу познакомить вас с компанией ${portal.company.name}. Возможно, вам будет полезно: «${mission.title.toLowerCase()}». Если актуально, организую короткое знакомство без обязательств.`; return <article key={mission.id}><span>{<bdi data-no-translate>{mission.programName}</bdi>}</span><h2>{<bdi data-no-translate>{mission.title}</bdi>}</h2><div className="knowledge-reward"><small>МОЖНО ЗАРАБОТАТЬ</small><strong>{mission.rewardLabel}</strong></div><blockquote>{intro}</blockquote><CopyTextButton text={intro} label="Скопировать сообщение" /><div><strong>Кого искать</strong><p>{<bdi data-no-translate>{mission.description}</bdi>}</p></div><div><strong>Что уточнить</strong>{mission.instructions.length ? <ul>{mission.instructions.map((item) => <li key={item}>{item}</li>)}</ul> : <p>Следуйте условиям задания.</p>}</div></article>; })}</section>
  </div>;
}
