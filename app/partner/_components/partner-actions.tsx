...
export function PartnerProfileForm({ token, partner, profile }: { token: string; partner: { email: string; phone: string }; profile: { firstName: string; lastName: string; middleName: string; instagram: string; avatarObjectKey: string | null; skills: string[]; industries: string[]; geographies: string[]; preferredTypes: string[] } }) {
  ...
  return <form className="partner-profile-form" onSubmit={save}><div className="partner-avatar-editor"><div>{avatarUrl ? <img src={avatarUrl} alt="Аватар партнёра" /> : <span>{initials}</span>}</div><label><strong>Аватар</strong><input name="avatar" type="file" accept="image/jpeg,image/png,image/webp" /><small>JPG, PNG или WEBP до 5 МБ</small></label></div><div className="partner-profile-fields"><label><span>Имя</span><input name="firstName" defaultValue={profile.firstName} required /></label><label><span>Фамилия</span><input name="lastName" defaultValue={profile.lastName} required /></label><label><span>Отчество</span><input name="middleName" defaultValue={profile.middleName} /></label><label><span>Телефон</span><input name="phone" type="tel" defaultValue={partner.phone} required /></label><label><span>Email</span><input name="email" type="email" defaultValue={partner.email} required /></label><label><span>Instagram</span><input name="instagram" defaultValue={profile.instagram} placeholder="username" /></label></div><label><span>География</span><input name="geographies" defaultValue={profile.geographies.join(", ")} placeholder="Казахстан, Алматы" /></label><label><span>Компетенции</span><textarea name="skills" defaultValue={profile.skills.join(", ")} rows={3} placeholder="B2B-продажи, CRM, автоматизация" /></label><label><span>Отрасли</span><textarea name="industries" defaultValue={profile.industries.join(", ")} rows={3} placeholder="IT, строительство, образование" /></label><fieldset><legend>Предпочитаемые миссии</legend>{types.map(([value, label]) => <label key={value}><input name="preferredTypes" value={value} type="checkbox" defaultChecked={profile.preferredTypes.includes(value)} />{label}</label>)}</fieldset><button className="button button-primary" type="submit">Сохранить профиль <span>→</span></button><p aria-live="polite">{notice}</p></form>;
}
