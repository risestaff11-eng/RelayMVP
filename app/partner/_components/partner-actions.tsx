...
type OpportunityMission = { id: string; type: string; title: string; description: string; rewardLabel: string; verificationRules: string; status: string; programName: string; programSlug: string; programExpiresAt: string | null };
...
export function OpportunityBrowser({ missions, acceptedMissionIds, token }: { missions: OpportunityMission[]; acceptedMissionIds: string[]; token: string }) {
  ...
  return <><div className="opportunity-filters"><label><span>Что хотите рекомендовать</span><select value={type} onChange={(event) => setType(event.target.value)}><option value="ALL">Все задания</option><option value="LEAD">Передать лид</option><option value="DEAL">Помочь со сделкой</option><option value="IMAGE">Поделиться кейсом</option><option value="ENGAGEMENT">Выполнить активность</option></select></label><small aria-live="polite">Показано заданий: {filtered.length}</small></div><section className="opportunity-grid">{filtered.map((mission) => { const deadline = mission.programExpiresAt ? new Date(mission.programExpiresAt).toLocaleDateString("ru-RU") : "Без дедлайна"; return <article className={`opportunity-card type-${mission.type.toLowerCase()}`} key={mission.id}><div><span>{typeNames[mission.type]}</span><small>● МОЖНО ЗАРАБОТАТЬ</small></div><small className="opportunity-program-name">{mission.programName}</small><h2>{mission.title}</h2><p>{mission.description}</p><dl><div><dt>Ваш заработок</dt><dd className="opportunity-reward-value">{mission.rewardLabel}</dd></div><div><dt>Дедлайн</dt><dd>{deadline}</dd></div><div><dt>Когда засчитают</dt><dd>{mission.verificationRules}</dd></div></dl><div className="opportunity-card-actions"><AcceptMissionButton token={token} missionId={mission.id} accepted={acceptedMissionIds.includes(mission.id)} resultHref={`/p/${mission.programSlug}/missions/${mission.id}/submit?access=${token}`} /></div></article>; })}</section>{!filtered.length && <section className="partner-large-empty"><span>◇</span><h2>Таких заданий пока нет</h2><p>Выберите другой тип — новые возможности появляются после публикации компанией.</p></section>}</>;
}
