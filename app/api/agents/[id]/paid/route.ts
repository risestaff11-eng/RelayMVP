...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await getDb().update(rewards).set({ status: paid ? "PAID" : "APPROVED", paidAt: paid ? now : null, updatedAt: now }).where(and(eq(rewards.partnerId, id), inArray(rewards.status, statuses)));
