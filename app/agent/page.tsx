...
export default async function AgentWorkspacePage() {
  ...
  return <main className="agent-workspace-page"><header><a href="https://risestaff.kz/" className="agent-access-brand"><i>R</i><b>RiseStaff</b></a><a href="/api/agent/logout">Выйти</a></header><section className="agent-company-picker" role="dialog" aria-labelledby="agent-company-picker-title"><div className="agent-workspace-intro"><small>ВЫБОР РАБОЧЕГО ПРОСТРАНСТВА</small><h1 id="agent-company-picker-title">{<bdi data-no-translate>{workspace.name}</bdi>}, с какой компанией работаем?</h1><p>Данные компаний не смешиваются. После выбора вы увидите только задания, отчёты, результаты и выплаты этой компании.</p></div><div className="agent-company-grid">{workspace.companies.map((company) => <article key={company.id}><header><i>{company.name.slice(0,1).toUpperCase()}</i><div><small>КОМПАНИЯ</small><h2>{<bdi data-no-translate>{company.name}</bdi>}</h2></div></header><div className="agent-company-programs">{company.programs.map((program) => <div key={program.id}><span><b>{<bdi data-no-translate>{program.name}</bdi>}</b><small>{program.historyOnly && <span>История и выплаты · </span>}{countRu(program.missionCount, "задание", "задания", "заданий")} · {countRu(program.submissionCount, "результат", "результата", "результатов")}</small></span>{program.pendingRewards.length > 0 && <strong>{formatMoneyGroups(program.pendingRewards)}</strong>}</div>)}</div><a href={`/api/agent/open?companyId=${encodeURIComponent(company.id)}`}>Перейти в кабинет компании <b>→</b></a></article>)}</div></section></main>;
}
