...
export default async function SubmissionsPage() {
  ...
  return <div className="dashboard-content module-content operations-page"><div className="module-heading"><div><span className="module-kicker">РЕЗУЛЬТАТЫ МИССИЙ</span><h1>Лиды и результаты</h1><p>Каждое решение сохраняется в истории партнёра. При отказе комментарий обязателен.</p></div><Link className="button button-ghost compact-button" href="/dashboard/programs">Открыть программы →</Link></div><section className="operations-metrics"><article><small>ВСЕГО РЕЗУЛЬТАТОВ</small><strong>{stats.submissions}</strong><span>За всё время</span></article><article><small>ЖДУТ ПРОВЕРКИ</small><strong>{stats.awaitingReview}</strong><span>Требуют решения</span></article><article><small>ПРИНЯТО</small><strong>{accepted}</strong><span>Готовы к работе</span></article><article><small>КОНВЕРСИЯ В СДЕЛКУ</small><strong>{items.length ? `${Math.round(items.filter((item) => item.status === "DEAL").length / items.length * 100)}%` : "—"}</strong><span>По текущим данным</span></article></section><section className="panel workflow-panel"><div className="panel-header"><div><h2>Очередь проверки</h2><p>Контакт раскрыт только компании; история решения видна партнёру.</p></div><CsvExportButton filename="relay-results.csv" headers={["Партнёр", "Программа", "Миссия", "Статус", "Дата"]} /></div><StatusFilters labels={[`Все · ${stats.submissions}`, "Отправлен", "Проверяется", "Принят", "В работе", "Сделка", "Отклонён"]} />{stats.submissions === 0 ? <div className="operations-empty"><div className="operations-empty-icon">↗</div><h3>Первый результат ещё не отправлен</h3><p>После публикации программы партнёр выберет миссию и передаст контакт или другое подтверждение.</p><Link className="button button-primary" href="/dashboard/programs">Подготовить программу <span>→</span></Link></div> : <SubmissionReviewList initialItems={items} />}</section></div>;
}
