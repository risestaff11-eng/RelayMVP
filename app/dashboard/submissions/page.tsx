...
export default async function SubmissionsPage({ searchParams }: { searchParams: Promise<{ submission?: string }> }) {
  ...
  return <div className="dashboard-content module-content operations-page">
    ...
    <aside className="antifraud-rule"><span>◎</span><div><strong>Контроль дублей и авторства включён</strong><p>Для лидов и сделок RiseStaff сверяет телефон и email по всей компании за 180 дней. Повторный контакт не создаётся под другим автором.</p></div></aside>
    <section className="panel workflow-panel"><div className="panel-header"><div><h2>Очередь заявок</h2><p>Проверка, движение продажи, авторство и выплата учитываются отдельно. Каждое изменение сохраняется в истории.</p></div><CsvExportButton filename="risestaff-applications.csv" label="Скачать таблицу" headers={["Агент", "Email агента", "Телефон агента", "Программа", "Задание", "Контакт", "Компания контакта", "Email контакта", "Телефон контакта", "Проверка", "Продажа", "Авторство", "Вознаграждение", "Валюта", "Дата"]} rows={exportRows} /></div>
