...
export default async function NotificationsPage() {
  ...
  return <div className="dashboard-content module-content operations-page notifications-page"><div className="module-heading"><div><span className="module-kicker">ЦЕНТР УВЕДОМЛЕНИЙ</span><h1>Что требует внимания</h1><p>Новые заявки, просроченные решения, выплаты и история изменений собраны в одном месте.</p></div></div><section className="notification-summary"><Link href="/dashboard/submissions"><small>ЖДУТ РЕШЕНИЯ</small><strong>{pending.length}</strong><span>{countRu(overdueReviews.length, "просрочена", "просрочены", "просрочено")}</span></Link><Link href="/dashboard/rewards"><small>ПРОСРОЧЕНЫ К ВЫПЛАТЕ</small><strong>{overduePayouts.length}</strong><span>По плановой дате или SLA 7 дней</span></Link><article><small>СОБЫТИЙ В ИСТОРИИ</small><strong>{recentEvents.length}</strong><span>Последние изменения заявок</span></article></section>{overdueReviews.length > 0 && <section className="panel notification-action-list"><div className="panel-header"><div><h2>Просрочена проверка</h2><p>Стандарт RiseStaff — принять решение по новой заявке за 48 часов.</p></div></div>{overdueReviews.map((item) => <Link href={`/dashboard/submissions?submission=${item.id}`} key={item.id}><span>!</span><div><strong>{item.contactName || item.contactCompany || "Заявка"}</strong><small>{item.programName} · {item.partnerName}</small></div><b>{slaState(item.reviewDueAt || null).label}</b></Link>)}</section>}{overduePayouts.length > 0 && <section className="panel notification-action-list"><div className="panel-header"><div><h2>Просрочена выплата</h2><p>Откройте реестр, отметьте перевод или уточните плановую дату.</p></div></div>{overduePayouts.map(({ reward, agent, mission }) => <Link href="/dashboard/rewards" key={reward.id}><span>¤</span><div><strong>{agent.name} · {formatMoney(reward.amount, reward.currency)}</strong><small>{mission.title}</small></div><b>{slaState(payoutDueAt(reward.approvedAt, reward.plannedAt)).label}</b></Link>)}</section>}<section className="panel notification-history"><div className="panel-header"><div><h2>История событий</h2><p>Решения не перезаписываются: RiseStaff сохраняет переходы и комментарии.</p></div></div>{recentEvents.length ? <div>{recentEvents.map(({ event, submission }) => <Link href={`/dashboard/submissions?submission=${submission.id}`} key={event.id}><i>●</i><span><strong>{submission.contactName || submission.contactCompany || "Заявка"}</strong><small>{event.comment || `${event.fromStatus || "Создана"} → ${event.toStatus}`} · {formatDateTime(event.createdAt)}</small></span><b>Открыть →</b></Link>)}</div> : <div className="table-empty">Событий пока нет.</div>}</section></div>;
}
