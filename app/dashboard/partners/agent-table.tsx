...
export function AgentTable({ initialAgents }: { initialAgents: Agent[] }) {
  ...
  async function changeStatus(agent: Agent) {
    ...
    if (nextStatus === "BLOCKED" && !window.confirm(localizeInterface(`Ограничить доступ агенту ${agent.name || agent.email}? Его персональная ссылка перестанет работать.`))) return;
    setPending(agent.id); setNotice("");
    try {
