...
export function CompanyBrand({ company }: { company: { id: string; name: string; logoObjectKey: string | null } }) {
  ...
  return <div className="company-white-label-brand-wrap"><div className="brand company-white-label-brand"><button type="button" className="company-logo-button" onClick={() => input.current?.click()} aria-label="Загрузить логотип компании">{logo ? <SiteImage unoptimized width={64} height={64} src={logo} alt={`Логотип ${company.name}`} /> : <span>{initials(company.name)}</span>}<i>＋</i></button><Link href="/dashboard" aria-label={`Вернуться в кабинет ${company.name}`}><strong>{<bdi data-no-translate>{company.name}</bdi>}</strong><small>Рабочее пространство</small></Link></div><input ref={input} hidden type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => void upload(event.target.files?.[0])} /><small className="company-logo-notice" aria-live="polite">{notice}</small></div>;
}

export function CompanyLogo({ company, className = "" }: { company: { id: string; name: string; logoObjectKey?: string | null }; className?: string }) {
  return <span className={`company-logo ${className}`}>{company.logoObjectKey ? <SiteImage unoptimized width={64} height={64} src={`/api/company/logo?companyId=${company.id}`} alt={`Логотип ${company.name}`} /> : initials(company.name)}</span>;
}
