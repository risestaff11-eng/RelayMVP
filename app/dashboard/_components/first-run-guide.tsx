      ...
      : !props.hasPublished
        ...
        : props.partnerCount === 0
          ...
          : props.submissionCount === 0
            ? { number: "05", title: "Помогите агенту передать первый результат", text: "Агент уже подключён. Он выбирает задание, передаёт контакт или подтверждение и видит статус в своём кабинете.", href: "/dashboard/agent-rating", action: "Открыть рейтинг агентов" }
            : { number: "04", title: "Новые заявки ждут решения", text: "Откройте карточку клиента и решите: взять в работу или отклонить с причиной.", href: "/dashboard/crm?quick=ACTION", action: `Открыть CRM${props.awaitingReview ? ` · ${props.awaitingReview}` : ""}` };
