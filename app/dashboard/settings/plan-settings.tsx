...
const plans = [
  { code: "TRIAL", name: "Старт", description: "Для проверки первого агентского канала.", features: ["Программы без ограничений в бете", "Кабинеты агентов", "50 000 AI-кредитов при регистрации"] },
...
export function PlanSettings({ user, company }: { user: { name: string; email: string }; company: { name: string; website: string; contactWhatsapp: string; contactInstagram: string; planCode: string; aiTokenBalance: number; aiTokensUsed: number } }) {
  ...
  const total = Math.max(50000, company.aiTokenBalance + company.aiTokensUsed);
