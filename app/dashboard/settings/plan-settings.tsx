...
export function PlanSettings({ user, company }: { user: { name: string; email: string }; company: { name: string; website: string; contactWhatsapp: string; contactInstagram: string; planCode: string; aiTokenBalance: number; aiTokensUsed: number; reviewSlaHours: number; payoutSlaDays: number } }) {
  ...
  return <div className="dashboard-content module-content settings-page">
    <div className="module-heading"><div><span className="module-kicker">ПРОФИЛЬ И ДОСТУП</span><h1>Настройки компании</h1><p>Аккаунт владельца, текущий тариф и запас RiseStaff.</p></div><div className="heading-actions settings-heading-actions"><Link className="button button-ghost compact-button" href="/dashboard/company-profile">← Вернуться к профилю компании</Link><Link className="button button-ghost compact-button" href="/dashboard/notifications">Уведомления</Link><Link className="button button-ghost compact-button" href="/dashboard/integrations">Интеграции</Link></div></div>
