...
export default async function ReportsPage(){const user=await requireChatGPTUser("/dashboard/reports");const company=await getCompanyForUser(user.userId);if(!company)redirect("/onboarding");if(!subscriptionAllows(company,"REPORTS"))return <PlanRequired/>;const[template,reports,overview,programs]=await Promise.all([ensureReportTemplate(company.id),getCompanyReports(company.id),getCompanyReportOverview(company.id),getProgramsForCompany(company.id)]);return <ReportsWorkspace companyName={company.name} template={template} initialReports={reports} overview={overview} programs={programs.map(({id,name})=>({id,name}))}/>}
