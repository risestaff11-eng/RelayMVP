"use client";
import { Clarification } from "@/app/partner/_components/clarification";
...
function LeadFullscreen({ companyName, members, item, pending, notice, onClose, onSave }: { companyName: string; members: { id: string; name: string }[]; item: CrmLead; pending: boolean; notice: string; onClose: () => void; onSave: (payload: Record<string, unknown>) => void }) {
  ...
  return <div className="relay-modal-backdrop crm-fullscreen-backdrop"><section className="crm-lead-fullscreen" role="dialog" aria-modal="true" aria-labelledby="crm-lead-title"><button className="relay-modal-close" type="button" onClick={onClose} aria-label="Закрыть">×</button>
    ...
    <div className="crm-fullscreen-body"><main>
      <section className="crm-detail-section"><div className="crm-section-heading"><small>ДАННЫЕ ЗАЯВКИ</small><h3>Что передал амбассадор</h3></div><div className="crm-detail-grid"><article><small>Компания клиента</small><strong>{(item.contactCompany) ? (<bdi data-no-translate>{item.contactCompany}</bdi>) : ("Не указана")}</strong></article><article><small>Email клиента</small><strong>{(item.contactEmail) ? (<bdi data-no-translate>{item.contactEmail}</bdi>) : ("Не указан")}</strong></article><article><small>Источник</small><strong>{sourceLabel(item)}</strong></article><article><small>Авторство</small><strong>{item.ownershipStatus === "CLEAR" ? "Закреплено" : "Требует проверки"}</strong></article></div><div className="crm-comment-block"><small>КОММЕНТАРИЙ АМБАССАДОРА</small><p>{(item.partnerComment) ? (<bdi data-no-translate>{item.partnerComment}</bdi>) : ("Комментарий не оставлен")}</p></div>{item.audioTranscript && <div className="crm-comment-block"><small>РАСШИФРОВКА ГОЛОСА</small><p>{<bdi data-no-translate>{item.audioTranscript}</bdi>}</p></div>}</section>
