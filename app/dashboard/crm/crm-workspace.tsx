...
export function CrmWorkspace({ companyName, initialItems, initialSettings, initialSelectedId = "" }: { companyName: string; initialItems: CrmLead[]; initialSettings: CrmSettings; initialSelectedId?: string }) {
  ...
  return <div className="crm-workspace">
    ...
    {selected && <LeadFullscreen companyName={companyName} item={selected} pending={pending === selected.id} notice={notice} onClose={() => { setSelected(null); setNotice(""); }} onMove={(stage) => move(selected, stage)} onSave={(payload) => patchLead(selected, payload, "Карточка клиента сохранена")} />}
  </div>;
}
...
function LeadFullscreen({ companyName, item, pending, notice, onClose, onMove, onSave }: { companyName: string; item: CrmLead; pending: boolean; notice: string; onClose: () => void; onMove: (stage: CrmStageId) => void; onSave: (payload: Record<string, unknown>) => void }) {
  ...
  return <div className="relay-modal-backdrop crm-fullscreen-backdrop"><section className="crm-lead-fullscreen" role="dialog" aria-modal="true" aria-labelledby="crm-lead-title"><button className="relay-modal-close" type="button" onClick={onClose} aria-label="Закрыть">×</button>
    <header className="crm-fullscreen-header"><div className="crm-fullscreen-context"><span>{item.programName}</span><strong>{item.missionTitle}</strong><time dateTime={item.createdAt}>{formatDateTime(item.createdAt)}</time></div><div className="crm-fullscreen-person"><div><small>КЛИЕНТ</small><h2 id="crm-lead-title">{leadTitle(item)}</h2>{clientWa ? <a href={clientWa} title="Открыть переписку" target="_blank" rel="noreferrer">{formatPhone(item.contactPhone)} ↗</a> : <span>{item.contactEmail || "Контакт не указан"}</span>}</div><div><small>ТЕКУЩИЙ ЭТАП</small><select value={crmStage(item)} disabled={pending} onChange={(event) => onMove(event.target.value as CrmStageId)}>{CRM_STAGES.map((stage) => <option value={stage.id} key={stage.id}>{stage.label}</option>)}</select><strong>{amount.amount ? `${amount.kind === "EXACT" ? "" : "≈ "}${formatMoney(amount.amount, item.currency)}` : "Сумма не задана"}</strong></div><div><small>ПРИВЁЛ</small><strong>{item.partnerName || item.partnerEmail}</strong>{agentWa ? <a href={agentWa} title="Открыть переписку" target="_blank" rel="noreferrer">{formatPhone(item.partnerPhone)} ↗</a> : <span>{item.partnerPhone || item.partnerEmail}</span>}</div></div></header>
    ...
    <div className="crm-fullscreen-body"><main>
      ...
      <section className="crm-payout-panel"><small>ВОЗНАГРАЖДЕНИЕ АМБАССАДОРУ</small><strong>{item.reward?.amount ? formatMoney(item.reward.amount, item.reward.currency) : item.rewardLabel || "Пока не рассчитано"}</strong><span className={payout.tone}>{payout.label}</span>{item.reward?.plannedAt && <p>Плановая дата: {shortDate(item.reward.plannedAt)}</p>}</section>
      <form className="crm-lead-form crm-inline-editor" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); onSave({ reviewStatus, salesStatus, estimatedDealAmount: data.get("estimatedDealAmount"), dealAmount: data.get("dealAmount"), amount: item.rewardValue, plannedAt: data.get("plannedAt"), comment: data.get("comment") }); }}><div className="crm-section-heading"><small>УПРАВЛЕНИЕ</small><h3>Данные компании</h3></div><div className="crm-status-pair"><label><span>Проверка</span><select value={reviewStatus} onChange={(event) => { const value = event.target.value; setReviewStatus(value); if (value !== "ACCEPTED") setSalesStatus(value === "REJECTED" ? "LOST" : "NONE"); }}>{Object.entries(reviewStatusNames).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label><span>Продажа</span><select value={salesStatus} disabled={reviewStatus !== "ACCEPTED"} onChange={(event) => setSalesStatus(event.target.value)}>{Object.entries(salesStatusNames).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label></div><div className="crm-status-pair"><label><span>Потенциальная сумма</span><input name="estimatedDealAmount" type="number" min="0" defaultValue={item.estimatedDealAmount || ""} placeholder="0" /></label><label><span>Фактически оплачено</span><input name="dealAmount" type="number" min="0" defaultValue={item.dealAmount || ""} placeholder="0" /></label></div><label><span>Плановая выплата</span><input name="plannedAt" type="date" defaultValue={item.reward?.plannedAt?.slice(0, 10) || ""} /></label><label><span>Комментарий компании</span><textarea name="comment" defaultValue={item.companyComment} rows={4} placeholder="Следующий шаг или договорённость" /></label><button className="button button-primary" disabled={pending} type="submit">{pending ? "Сохраняем…" : "Сохранить изменения"}</button></form>
      <section className="crm-service-state"><span><small>Проверка</small><strong>{reviewStatusNames[item.reviewStatus as keyof typeof reviewStatusNames] || item.reviewStatus}</strong></span><span><small>Продажа</small><strong>{salesStatusNames[item.salesStatus as keyof typeof salesStatusNames] || item.salesStatus}</strong></span><span><small>SLA проверки</small><strong>{slaState(item.reviewDueAt, !["PENDING", "REVIEWING"].includes(item.reviewStatus)).label}</strong></span></section>
    </aside></div>
  </section></div>;
}
