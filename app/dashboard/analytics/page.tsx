...
export default async function AnalyticsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <div className="dashboard-content module-content operations-page agent-analytics-page">
          ...
          <div className="agent-ranking">
            {rankedAgents.map((agent, index) => (
              <article className="agent-rank-row" key={agent.id}>
                ...
                <div className="agent-rank-person"><strong>{(agent.name) ? (<bdi data-no-translate>{agent.name}</bdi>) : (<bdi data-no-translate>{agent.email}</bdi>)}</strong><small>{<bdi data-no-translate>{agent.programName}</bdi>}</small></div>
      ...
      {inactiveAgents.length > 0 && <section className="panel attention-agents-panel"><div className="panel-header"><div><h2>Кого стоит активировать</h2><p>Агенты без заявок в выбранном периоде. Свяжитесь с ними или предложите более подходящее задание.</p></div></div><div>{inactiveAgents.map((agent) => { const digits = agent.phone.replace(/\D/g, ""); const message = encodeURIComponent(`Здравствуйте, ${agent.name || "коллега"}! В программе «${agent.programName}» появились актуальные задания. Подскажите, нужна ли помощь с первой заявкой?`); return <article key={agent.id}><span><strong>{(agent.name) ? (<bdi data-no-translate>{agent.name}</bdi>) : (<bdi data-no-translate>{agent.email}</bdi>)}</strong><small>{<bdi data-no-translate>{agent.programName}</bdi>} · активность {formatDate(agent.lastActivity)}</small></span>{digits ? <a href={`https://wa.me/${digits}?text=${message}`} target="_blank" rel="noreferrer">Написать в WhatsApp ↗</a> : <a href={`mailto:${agent.email}`}>Написать на email ↗</a>}</article>; })}</div></section>}
    </div>
  );
}
