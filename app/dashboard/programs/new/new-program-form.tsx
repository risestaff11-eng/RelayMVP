...
export function NewProgramForm({ companyName, tokenBalance, profileVersion, profileStatus }: { companyName: string; tokenBalance: number; profileVersion: number | null; profileStatus: string }) {
  ...
  const [name, setName] = useState(`Агентская кампания ${companyName}`);
  ...
  async function generate(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok || !data.programId) throw new Error(data.error || "Не удалось сгенерировать кампанию");
      router.push(`/dashboard/programs/${data.programId}`);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Не удалось сгенерировать кампанию");
      setPending(false);
    }
  }
  ...
  return (
    <div className="dashboard-content module-content program-builder-page">
      <div className="builder-back"><Link href="/dashboard/programs">← Все кампании</Link><span>{profileVersion ? `AI-профиль v${profileVersion}${profileStatus === "CONFIRMED" ? " · подтверждён" : " · черновик"}` : "AI-профиль не заполнен"} · {tokenBalance.toLocaleString("ru-RU")} токенов</span></div>
      <div className="module-heading"><div><span className="module-kicker">НОВАЯ КАМПАНИЯ · ШАГ 1 ИЗ 3</span><h1>Задайте рамки кампании</h1><p>Gemini использует доступные данные компании и создаст редактируемый черновик каждого выбранного задания. Подтверждение профиля не обязательно.</p></div></div>
      ...
      <form className="program-create-layout" onSubmit={generate}>
        <section className="panel program-basics-card">
          <div className="panel-header"><div><h2>Основа кампании</h2><p>Название увидят агенты по внешней ссылке.</p></div></div>
          <label className="builder-field"><span>Название кампании</span><input value={name} onChange={(event) => setName(event.target.value)} maxLength={100} required /></label>
          <div className="builder-field-row"><label className="builder-field"><span>Главная цель</span><select value={goal} onChange={(event) => setGoal(event.target.value)}><option value="MIXED">Смешанная кампания</option><option value="LEADS">Больше квалифицированных лидов</option><option value="DEALS">Больше оплаченных сделок</option><option value="BRAND">Узнаваемость и доверие</option><option value="ENGAGEMENT">Вовлечение агентов</option></select></label><label className="builder-field"><span>Валюта</span><select value={currency} onChange={(event) => setCurrency(event.target.value)}><option value="KZT">KZT · ₸</option><option value="RUB">RUB · ₽</option><option value="USD">USD · $</option><option value="EUR">EUR · €</option></select></label></div>
        </section>
