  ...
  return (
    <div className="dashboard-content module-content program-builder-page">
      ...
      <form className="program-create-layout" onSubmit={generate}>
        ...
        <div className="builder-submit-bar"><div><strong>Что сделает Gemini</strong><p>Предложит действия, доказательства результата, правила проверки и стартовую награду. Списание — по факту, не более {300 + selected.length * 150} AI-кредитов.</p></div><button className="button button-primary" type="submit" disabled={pending || selected.length === 0}>{pending ? "Gemini создаёт задания…" : "Сгенерировать черновик"}<span>✦</span></button></div>
      </form>
    </div>
  );
}
