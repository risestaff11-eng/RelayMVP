...
function rewardText(mission: MissionRecord, currency: string) {
  ...
  return mission.rewardValue ? `${mission.rewardValue.toLocaleString("ru-RU")} ${symbol}` : "Вознаграждение не указано";
}
...
export function ProgramEditor({ initialProgram }: { initialProgram: ProgramRecord }) {
  ...
  function updateProgram(field: keyof ProgramRecord, value: string | boolean) { setProgram((current) => ({ ...current, [field]: value })); }
  ...
  async function removeMission(mission: MissionRecord) {
    ...
    if (!window.confirm(localizeInterface(`Удалить задание «${mission.title}»?`))) return;
    try {
  ...
  async function callRiseStaff(body: Record<string, unknown>) {
    ...
    if (!response.ok) throw new Error(data.error || "RiseStaff не смог подготовить вариант");
    return data;
  }
  ...
  async function generateMissionDraft() {
    ...
    try {
      const data = await callRiseStaff({ action: "MISSION", missionType: missionDraft.type, currentMissionId: missionDraft.id });
      if (!data.mission) throw new Error("RiseStaff вернула неполный вариант задания");
  ...
  async function removeResource(resourceId: string) {
    if (!missionDraft || !window.confirm(localizeInterface("Удалить файл из задания?"))) return;
  ...
  async function generateForm() {
    ...
    setAiPending("form");
    try { const data = await callRiseStaff({ action: "FORM" }); if (!data.formFields) throw new Error("RiseStaff вернула неполную форму"); setFormDraft(data.formFields); }
  ...
  async function fillFieldWithAi() {
    setAiPending("field");
    try { const data = await callRiseStaff({ action: "FIELD", stage: fieldDraft.stage, type: fieldDraft.type, scope: fieldDraft.scope }); if (!data.field) throw new Error("RiseStaff не смог заполнить поле"); setFieldDraft((current) => ({ ...current, ...data.field, id: current.id })); }
  ...
  function validateForPublish() {
    ...
    if (completedMissionCount !== program.missions.length) return "Завершите настройку всех заданий: цель, шаги, подтверждение, награда и правила проверки.";
