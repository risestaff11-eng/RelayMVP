...
export function ProgramEditor({ initialProgram }: { initialProgram: ProgramRecord }) {
  ...
  async function generateForm() {
    if (!window.confirm("RiseStaff заменит текущий черновик формы новым вариантом. Продолжить?")) return;
    setAiPending("form");
  ...
  async function downloadPoster() {
    ...
    const canvas = document.createElement("canvas"); canvas.width = 1240; canvas.height = 1754; const context = canvas.getContext("2d"); if (!context) return; context.fillStyle = "#f4f2ea"; context.fillRect(0, 0, canvas.width, canvas.height); context.fillStyle = "#b8ff32"; context.fillRect(0, 0, canvas.width, 235); context.fillStyle = "#171914"; context.font = "800 42px Arial"; context.fillText("RiseStaff", 90, 105); context.font = "700 24px Arial"; context.fillText("АГЕНТСКАЯ ПРОГРАММА", 90, 160); context.font = "700 58px Arial"; context.fillText(program.name.length > 36 ? `${program.name.slice(0, 34)}…` : program.name, 90, 330); context.font = "32px Arial"; context.fillText("Сканируйте QR-код и выберите задание", 90, 390);
