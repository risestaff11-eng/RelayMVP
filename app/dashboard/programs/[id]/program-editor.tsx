...
export function ProgramEditor({ initialProgram }: { initialProgram: ProgramRecord }) {
  ...
  async function persist(action: "save" | "publish" | "pause") {
    ...
    try {
      ...
      setNotice({ type: "success", text: action === "publish" ? "Программа опубликована. Ссылка готова для партнёров." : action === "pause" ? "Программа поставлена на паузу." : "Черновик программы сохранён." });
    } catch (reason) {
  ...
  return (
    <div className="dashboard-content module-content program-builder-page">
      ...
      <div className="builder-stepper"><span className="done"><b>✓</b>Основа</span><span className="active"><b>2</b>Миссии и награды</span><span className={program.status === "ACTIVE" ? "done" : ""}><b>{program.status === "ACTIVE" ? "✓" : "3"}</b>Правила и публикация</span></div>
      {notice && <div className={`inline-notice ${notice.type}`} role="status">{notice.text}</div>}

      <section className="panel program-general-card">
        <div className="panel-header"><div><h2>Описание программы</h2><p>Коротко объясните партнёру, что он может рекомендовать и какой результат ожидается.</p></div></div>
      ...
      <div className="mission-editor-heading"><div><span className="module-kicker">ШАГ 2</span><h2>Миссии и вознаграждения</h2><p>Награда различается по типу результата. AI предложил стартовые значения — ответственность за финальные условия остаётся у компании.</p></div><span>{program.missions.length} карточки</span></div>
      <section className="mission-editor-grid">{program.missions.map((mission, index) => <article className={`panel mission-editor-card type-${mission.type.toLowerCase()}`} key={mission.id}><div className="mission-editor-top"><span>0{index + 1} · {typeNames[mission.type]}</span><i>{mission.type === "LEAD" ? "↗" : mission.type === "DEAL" ? "◇" : mission.type === "IMAGE" ? "◎" : "✦"}</i></div><label className="builder-field"><span>Название миссии</span><input value={mission.title} onChange={(event) => updateMission(mission.id, "title", event.target.value)} /></label><label className="builder-field"><span>Что нужно получить</span><textarea rows={3} value={mission.description} onChange={(event) => updateMission(mission.id, "description", event.target.value)} /></label><label className="builder-field"><span>Шаги партнёра · по одному в строке</span><textarea rows={5} value={mission.instructions.join("\n")} onChange={(event) => updateMission(mission.id, "instructions", event.target.value.split("\n").map((item) => item.trim()).filter(Boolean))} /></label><label className="builder-field"><span>Что приложить для проверки</span><textarea rows={3} value={mission.proofRequirements.join("\n")} onChange={(event) => updateMission(mission.id, "proofRequirements", event.target.value.split("\n").map((item) => item.trim()).filter(Boolean))} /></label><div className="reward-editor"><label className="builder-field"><span>Тип награды</span><select value={mission.rewardMode} onChange={(event) => updateMission(mission.id, "rewardMode", event.target.value)}><option value="FIXED">Фиксированная сумма</option><option value="PERCENT">Процент</option><option value="POINTS">Баллы</option><option value="NON_MONETARY">Неденежная</option></select></label><label className="builder-field"><span>Значение</span><input type="number" min="0" value={mission.rewardValue} onChange={(event) => updateMission(mission.id, "rewardValue", Number(event.target.value))} /></label></div><label className="builder-field"><span>Как увидит партнёр</span><input value={mission.rewardLabel} onChange={(event) => updateMission(mission.id, "rewardLabel", event.target.value)} placeholder={`Например: 25 000 ${program.currency}`} /></label><label className="builder-field"><span>Правила проверки</span><textarea rows={3} value={mission.verificationRules} onChange={(event) => updateMission(mission.id, "verificationRules", event.target.value)} /></label></article>)}</section>

      <section className="panel publication-card">
        <div className="panel-header"><div><span className="module-kicker">ШАГ 3</span><h2>Выплата, ограничения и публикация</h2><p>Эти условия видит партнёр до выполнения миссии. Не оставляйте их двусмысленными.</p></div></div>
