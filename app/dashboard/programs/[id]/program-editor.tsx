...
export function ProgramEditor({ initialProgram }: { initialProgram: ProgramRecord }) {
  ...
  async function callYaler(body: Record<string, unknown>) {
    ...
    if (!response.ok) throw new Error(data.error || "Yaler не смог подготовить вариант");
    return data;
  }
  ...
  async function generateMissionDraft() {
    ...
    try {
      const data = await callYaler({ action: "MISSION", missionType: missionDraft.type, currentMissionId: missionDraft.id });
      if (!data.mission) throw new Error("Yaler вернула неполный вариант задания");
  ...
  async function generateForm() {
    if (!window.confirm("Yaler заменит текущий черновик формы новым вариантом. Продолжить?")) return;
    setAiPending("form");
    try { const data = await callYaler({ action: "FORM" }); if (!data.formFields) throw new Error("Yaler вернула неполную форму"); setFormDraft(data.formFields); }
  ...
  async function fillFieldWithAi() {
    setAiPending("field");
    try { const data = await callYaler({ action: "FIELD", stage: fieldDraft.stage, type: fieldDraft.type, scope: fieldDraft.scope }); if (!data.field) throw new Error("Yaler не смог заполнить поле"); setFieldDraft((current) => ({ ...current, ...data.field, id: current.id })); }
  ...
  async function downloadPoster() {
    ...
    const canvas = document.createElement("canvas"); canvas.width = 1240; canvas.height = 1754; const context = canvas.getContext("2d"); if (!context) return; context.fillStyle = "#f4f2ea"; context.fillRect(0, 0, canvas.width, canvas.height); context.fillStyle = "#b8ff32"; context.fillRect(0, 0, canvas.width, 235); context.fillStyle = "#171914"; context.font = "800 42px Arial"; context.fillText("Yaler", 90, 105); context.font = "700 24px Arial"; context.fillText("АГЕНТСКАЯ ПРОГРАММА", 90, 160); context.font = "700 58px Arial"; context.fillText(program.name.length > 36 ? `${program.name.slice(0, 34)}…` : program.name, 90, 330); context.font = "32px Arial"; context.fillText("Сканируйте QR-код и выберите задание", 90, 390);
