...
test("renders the RiseStaff landing page", async () => {
  ...
  assert.match(html, /RiseStaff: клиенты по рекомендациям, сделки и выплаты/);
  ...
  assert.match(html, /rs-site/);
  assert.match(html, /рекомендации/);
  assert.match(html, /Одна заявка\. Вся история до выплаты/);
  assert.doesNotMatch(html, /Northstar CRM|внедрение CRM/i);
  assert.match(html, /Контакт, сделка и вознаграждение в одном месте/);
  assert.doesNotMatch(html, /ТЕКУЩИЙ ПИЛОТ RISESTAFF/);
  assert.match(html, /не продаёт базу агентов/);
  assert.match(html, /Не теряйте клиентов/);
  assert.match(html, /Пример для учебного центра/);
  assert.doesNotMatch(html, /B2B-клиент|B2B-лид|B2B-продаж/i);
  assert.doesNotMatch(html, /стабильный источник|lp-marquee-track|2 400 000/);
  assert.doesNotMatch(html, /ранний доступ|бета-тест/i);
  assert.match(html, /Что важно знать/);
