...
test("renders the Yaler landing page", async () => {
  ...
  assert.match(html, /Yaler — продажи через агентов и амбассадоров/);
  ...
  assert.match(html, /ТЕКУЩИЙ ПИЛОТ YALER/);
  assert.match(html, /Yaler не является биржей амбассадоров/);
