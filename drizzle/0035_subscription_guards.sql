CREATE TRIGGER program_subscription_capacity BEFORE UPDATE OF status ON programs
...
BEGIN
  ...
  ) THEN RAISE(ABORT, 'PROGRAM_LIMIT_REACHED') END;
END;
--> statement-breakpoint
CREATE TRIGGER product_company_created AFTER INSERT ON companies BEGIN
  INSERT OR IGNORE INTO product_milestones VALUES (NEW.id, 'company_created', strftime('%Y-%m-%dT%H:%M:%fZ','now'));
END;
--> statement-breakpoint
CREATE TRIGGER product_program_published AFTER UPDATE OF status ON programs WHEN NEW.status = 'ACTIVE' AND OLD.status <> 'ACTIVE' BEGIN
  INSERT OR IGNORE INTO product_milestones VALUES (NEW.company_id, 'first_program_published', strftime('%Y-%m-%dT%H:%M:%fZ','now'));
END;
--> statement-breakpoint
CREATE TRIGGER product_agent_joined AFTER INSERT ON partners BEGIN
  INSERT OR IGNORE INTO product_milestones VALUES (NEW.company_id, 'first_agent_joined', strftime('%Y-%m-%dT%H:%M:%fZ','now'));
END;
--> statement-breakpoint
CREATE TRIGGER product_submission_created AFTER INSERT ON submissions BEGIN
  INSERT OR IGNORE INTO product_milestones VALUES (NEW.company_id, 'first_submission', strftime('%Y-%m-%dT%H:%M:%fZ','now'));
END;
--> statement-breakpoint
CREATE TRIGGER product_sale_completed AFTER UPDATE OF sales_status ON submissions WHEN NEW.sales_status = 'WON' AND OLD.sales_status <> 'WON' BEGIN
  INSERT OR IGNORE INTO product_milestones VALUES (NEW.company_id, 'first_sale', strftime('%Y-%m-%dT%H:%M:%fZ','now'));
END;
--> statement-breakpoint
CREATE TRIGGER product_reward_confirmed AFTER UPDATE OF partner_confirmed_at ON rewards WHEN NEW.partner_confirmed_at IS NOT NULL AND OLD.partner_confirmed_at IS NULL BEGIN
  INSERT OR IGNORE INTO product_milestones VALUES (NEW.company_id, 'first_reward_confirmed', strftime('%Y-%m-%dT%H:%M:%fZ','now'));
END;
--> statement-breakpoint
CREATE TRIGGER product_subscription_paid AFTER INSERT ON subscription_events WHEN NEW.paid_amount > 0 AND NEW.action IN ('ACTIVATE','EXTEND') BEGIN
